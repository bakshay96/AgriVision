  ...
  try {
    ...
    const forecast = Array.from(dailyForecasts.entries())
      ...
      .map(([dateStr, items], i) => {
        ...
        const avgWind = items.reduce((sum: number, item: any) => sum + item.wind.speed, 0) / items.length * 3.6;
        const rainProbability = items.reduce((sum: number, item: any) => sum + (item.pop || 0), 0) / items.length * 100;
        const rainAmount = items.reduce((sum: number, item: any) => sum + (item.rain?.['3h'] || 0), 0);
    ...
    if (current.condition === 'Thunderstorm' || current.condition === 'Rain') {
      alerts.push({
        ...
        severity: current.condition === 'Thunderstorm' ? 'high' as const : 'medium' as const,
