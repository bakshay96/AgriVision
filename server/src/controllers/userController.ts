...
export const getSelectedCrops = async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user?._id;
    
    const user = await User.findById(userId).select('selectedCrops');
...
export const addSelectedCrop = async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user?._id;
...
export const removeSelectedCrop = async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user?._id;
...
export const updateSelectedCrops = async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user?._id;
    const { crops } = req.body;

    if (!Array.isArray(crops)) {
      throw createError('Crops must be an array', 400);
    }

    if (crops.length > 5) {
      throw createError('Maximum 5 crops allowed', 400);
    }
