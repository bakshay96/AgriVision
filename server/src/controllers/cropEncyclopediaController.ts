...
import CropEncyclopedia from '../models/CropEncyclopedia';
...
const sampleCrops = [
  {
    ...
    origin: 'Asia',
    description:
      'Rice is a staple food crop for more than half of the world\'s population. It is primarily grown in flooded fields called paddies.',
    category: 'cereal',
...
export const aiSearchCrop = async (req: AuthRequest, res: Response): Promise<void> => {
  const { cropName, language = 'en' } = req.body;

  if (!cropName || typeof cropName !== 'string' || cropName.trim().length === 0) {
    throw createError('cropName is required', 400);
  }

  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) {
    throw createError('AI service is not configured on this server', 503);
  }
...
Ensure commonPests has exactly 2 entries and stages has 4-5 distinct phases. All string values (not keys) must be in ${langLabel}.`;
  ...
  throw createError('AI crop search failed. Please try again.', 502);
};

// ─── Get all crops ─────────────────────────────────────────────────────────────
export const getAllCrops = async (req: AuthRequest, res: Response): Promise<void> => {
  const tenantId = req.tenantId!;
  const { search, category, page = 1, limit = 20 } = req.query;

  const existingCount = await CropEncyclopedia.countDocuments({ tenantId });
  if (existingCount === 0) {
    const cropsWithTenant = sampleCrops.map((crop) => ({ ...crop, tenantId, isActive: true }));
    await CropEncyclopedia.insertMany(cropsWithTenant);
  }
  ...
  const [crops, total, categories] = await Promise.all([
    CropEncyclopedia.find(filter)
      ...
      .limit(Number(limit))
      .sort({ name: 1 }),
    CropEncyclopedia.countDocuments(filter),
    CropEncyclopedia.distinct('category', { tenantId, isActive: true }),
  ]);
...
export const getCropById = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  const crop = await CropEncyclopedia.findOne({ _id: id, tenantId });
...
// ─── Search crops ──────────────────────────────────────────────────────────────
export const searchCrops = async (req: AuthRequest, res: Response): Promise<void> => {
  const tenantId = req.tenantId!;
  const { q } = req.query;

  if (!q) throw createError('Search query is required', 400);

  const crops = await CropEncyclopedia.find(
    { tenantId, isActive: true, $text: { $search: q as string } },
...
// ─── Get crop by name ──────────────────────────────────────────────────────────
export const getCropByName = async (req: AuthRequest, res: Response): Promise<void> => {
  const tenantId = req.tenantId!;
  const { name } = req.params;

  const crop = await CropEncyclopedia.findOne({ tenantId, name: new RegExp(name, 'i'), isActive: true });
  if (!crop) throw createError('Crop not found', 404);
...
export const getPestsAndDiseases = async (req: AuthRequest, res: Response): Promise<void> => {
  const tenantId = req.tenantId!;
  const { id } = req.params;

  const crop = await CropEncyclopedia.findOne({ _id: id, tenantId }, { pestsAndDiseases: 1, name: 1 });
  if (!crop) throw createError('Crop not found', 404);
...
export const getCropAdvice = async (req: AuthRequest, res: Response): Promise<void> => {
  const tenantId = req.tenantId!;
  const { id } = req.params;
