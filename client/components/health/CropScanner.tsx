  ...
  return (
    ...
    >
      ...
      <div className={`${cfg.bg} p-4 space-y-4`}>
        ...
        {/* Required Nutrients */}
        {result.requiredNutrients && result.requiredNutrients.length > 0 && (
          <div className="rounded-xl border border-white/60 bg-white/60 p-3">
            <div className="flex items-center gap-1.5 mb-2">
            ...
            <div className="flex flex-wrap gap-2">
              {result.requiredNutrients.map((n, i) => (
                <span key={i} className="rounded-lg bg-purple-100 px-2.5 py-1 text-xs font-semibold text-purple-700 border border-purple-200 shadow-sm">{n}</span>
              ))}
            </div>
          </div>
        )}
        ...
        {/* Spray Instructions */}
        {result.sprayInstructions && (
          <div className="rounded-xl border border-white/60 bg-white/60 p-3">
            <div className="flex items-center gap-1.5 mb-1.5">
              <Activity className="h-4 w-4 text-blue-500" />
              <p className="text-[10px] font-semibold text-slate-600 uppercase tracking-wider">
                Dosage & Spray Instructions (Per Acre)
              </p>
            </div>
            <p className="text-xs leading-relaxed text-slate-700 bg-blue-50/50 p-2.5 rounded-lg border border-blue-100">{result.sprayInstructions}</p>
          </div>
        )}
        ...
        {/* Footer meta */}
        <div className="flex items-center justify-between border-t border-white/50 pt-2 flex-wrap gap-2">
          ...
          <div className="flex items-center gap-3">
            {result.estimatedRecoveryDays !== undefined && result.estimatedRecoveryDays > 0 && (
              <span className="flex items-center gap-1 text-[11px] text-slate-500 font-medium">
                <Activity className="h-3 w-3 text-amber-500" />
                ~{result.estimatedRecoveryDays}d recovery
              </span>
            )}
...
export default function CropScanner({ onScanComplete, cropId }: CropScannerProps) {
  ...
  // New state variables for crop details
  const [cropName, setCropName] = useState<string>('');
  const [customCropName, setCustomCropName] = useState<string>('');
  const [cropAge, setCropAge] = useState<string>('');
  ...
  // Validate form
  const isFormValid = cropName && (cropName !== 'Other' || customCropName.trim() !== '') && cropAge.trim() !== '' && description.trim() !== '';
  ...
  const { mutate: scan, isPending: isScanning } = useMutation({
    mutationFn: (file: File) => {
      ...
      fd.append('cropName', finalCropName);
      fd.append('cropAge', cropAge);
    ...
    onError: (err: unknown) => {
      ...
      setFileError(msg); // Set the actual error message in UI
      setPreview(null);
    },
  });
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    maxFiles: 1,
    maxSize: MAX_SIZE,
    disabled: isScanning || !Boolean(isFormValid),
  });
          ...
          >
            ...
            <div className="mb-6 space-y-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="text-sm font-bold text-slate-700">Crop Details First</h3>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-semibold text-slate-600 mb-1.5 block">Crop Name <span className="text-red-500">*</span></label>
                  <select 
                    value={cropName} 
                    onChange={(e) => setCropName(e.target.value)}
                    disabled={isScanning}
                    className="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500 bg-slate-50 transition-colors"
                  >
                ...
                {cropName === 'Other' && (
                  <div>
                    <label className="text-xs font-semibold text-slate-600 mb-1.5 block">Specify Crop <span className="text-red-500">*</span></label>
                    <input 
                      type="text" 
                ...
                <div className={cropName !== 'Other' ? 'col-span-1 border-none' : 'col-span-1 md:col-span-2 border-none'}>
                  <label className="text-xs font-semibold text-slate-600 mb-1.5 block">Crop Age <span className="text-red-500">*</span></label>
                  <input 
                    type="text" 
                    value={cropAge} 
              ...
              <div>
                <label className="text-xs font-semibold text-slate-600 mb-1.5 block">Description / Context <span className="text-red-500">*</span></label>
                <textarea 
                  value={description} 
                  onChange={(e) => setDescription(e.target.value)}
                  placeholder="Describe what you're observing (e.g. Yellowing leaves with brown spots starting from the bottom...)"
                  disabled={isScanning}
                  className="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500 bg-slate-50 transition-colors resize-none mb-1"
                  rows={2}
                />
              </div>
            </div>
            ...
            <div
              ...
              className={`relative cursor-pointer rounded-2xl border-2 border-dashed p-8 text-center transition-all select-none ${isScanning
                  ...
                  : !isFormValid 
                    ? 'cursor-not-allowed border-slate-200 bg-slate-100 opacity-60 grayscale-[10%]'
                    : isDragActive
            ...
            >
              ...
              {preview ? (
                ...
                <div className="space-y-3">
                  <motion.div
                    animate={isDragActive && isFormValid ? { scale: 1.12 } : { scale: 1 }}
                    className={`mx-auto flex h-16 w-16 items-center justify-center rounded-2xl border ${isFormValid ? 'bg-emerald-50 border-emerald-100' : 'bg-slate-200 border-slate-300'}`}
                  >
                    {isScanning ? (
                      <Loader2 className="h-7 w-7 animate-spin text-emerald-600" />
                    ) : (
                      <Upload className={`h-7 w-7 ${isFormValid ? 'text-emerald-600' : 'text-slate-400'}`} />
                    )}
                  </motion.div>

                  <div>
                    <p className={`text-sm font-semibold ${isFormValid ? 'text-slate-700' : 'text-slate-500'}`}>
                      {!isFormValid ? 'Fill details above to upload image' : isDragActive ? 'Drop the image here' : 'Drop crop photo or click to browse'}
                    </p>
